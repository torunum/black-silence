...
says so in §10 and leaves the decision where it belongs. It changes nothing.
...
- **No level layout work.** Carving, room shapes, `hmap` tiering and the five
  hand-built maps are Phase 4's. Phase 3 touches level grids only if §7's
  decision says it must, and then only as placement substitution.
- **No boss implementation.** Phase 3 *specifies* the three brains and deletes
  ...
  themselves to Phase 5. §5 exists so Phase 5 inherits a shape, not a surprise.
...
- **No fix for KNOWN-4 or KNOWN-11.** Both are glyph collisions between the
  ...
  *changes which letters collide* — see §7 — but does not resolve them.
- **No "fix KNOWN-15" as a single commit.** That row says so itself and §6 is
...
## 3. Ground truth: what the roster actually is
...
### 3.1 Enemy letters placed, per level
...
### 3.2 The shape of the cut, stated honestly
...
### 3.3 Other counts this spec depends on, re-derived
...
| Baked canvas textures at boot | **446** |
| `material.map=` assignment sites in `src/` | **9** — `ai/Behaviors.ts` ×4, `Boss.ts` ×2, `Damage.ts`, `Death.ts`, `player/Interact.ts` |

The 446 is worth one footnote: `direction.md`'s sprite amendment estimates "~14
textures each, about 434". Fourteen is the count for an entry with a head
region; nine entries have none, and ten carry the optional attack and two death
frames. The estimate was close and its argument is unaffected — the real figure
is slightly *higher*.

---

## 4. The nine, mapped to what exists
...
|---|---|---|---|---|
| **The Flock** | Crowd control — shotgun fodder | `z` (50 hp, the commonest letter: 13 of 73 non-boss tiles in kept levels) | — | Sprite and name; already the priest's summon, so it is the one letter two systems depend on by name |
| **The Fleet** | Target prioritization | `f` (35 hp, fastest ground unit, `dodge`) | — | Sprite and name only. Nothing dead is authored on it |
| **The Hound** | Aim down | `g` (30 hp, `lunge`, the only wide-and-short ground def) | — | Sprite and name only |
| **The Penitent** | Flank it or kick it | `m` (`plate:45`) | `k` (`shield`) | The two answers merge; see §6, this is the one merge that needs a dead field turned on |
...
| **The Crawler** | Watch the ground | `w` (30 hp, the lowest def in the table) | — | Sprite and name only; today it has no flag at all, which is exactly right for "it is just *low*" |
...
| **The Suspended** | Look up — **the only flier** | `C` (120 hp, `fly`, `flyH:1.7`, `orb`, `range:15` — the middle stat block of the five) | `L`, `q`, `R`, `y` | Five defs become one. The largest single deletion in the phase and the only one that needs two dead fields on |

That accounts for fourteen of the twenty-five letters. The rest:

- **`E`, `U`, `Q`, `Z`, `N`, `H`, `V`, `G`** — the eight bosses, resolved in §5.
- **`A`** (Mancubus, 260 hp, `twin`) and **`j`** (Cultist, 45 hp, `range`,
  `dodge`, `orb`) have **no successor among the nine**. `A` is the second most
  placed letter in the kept levels (9 tiles) and `j` is placed once. See §10.1 —
  this is a real gap in the input, not a decision this spec makes.
...
## 5. The bosses — three brains replacing one
...
with no successor among the named three; see §10.2.
...
### 5.1 The risk: the boss brain has no trace coverage
...
## 6. KNOWN-15 — which fields the new roster actually needs
...
| **`orb`** | `C A j k q R y` | **Required — The Suspended**, and only it | The smallest possible version of this field's fix: one branch of `fireOrb` instead of seven. Today every one of the seven falls through to a default purple 15-damage, 9.5-speed bolt. Turning it on for one enemy changes exactly one projectile's colour, damage and speed. **Turning it on for all seven at once is the balance change KNOWN-15 warns about** |
...
| **`charger`** | `L` | **Owner's call, default off.** The Lost Soul's telegraphed dash | If The Suspended dives, the forced behavior becomes "dodge a dash" — which is the Hound's answer, and the roster rule says the duplicate dies. Turning it on is a deliberate choice to make the flier's answer two things instead of one |
| **`twin`** | `A` | **Not required — `A` has no successor.** Delete with the def | `A`'s second barrel is a 220 ms-delayed second orb. It only means anything on a heavy artillery unit, and the nine have none (§10.1) |
...
`twin`, `burst`, `sovereign`, and `charger` unless §10 says otherwise), and
...
## 7. The level coupling — the decision this spec does not make
...
- **Costs:** the largest phase in the roadmap by a wide margin, with no
  ...
  level authoring — under one review loop, where this project's record says the
  loop earns its keep by being *narrow*. Nothing ships until both halves do.
...
- **Buys:** the game plays at every commit, the level tables stay honest, and
  ...
  which, given §2, is the only way they become visible at all.
...
## 8. Sprites — what the cut buys, and what it does not
...
Measured today: **31 `PXDEF` entries, 446 baked canvas textures at boot.** After
...
So the boot-time texture count falls to roughly a third, and the manual art
falls with it: **seven new angles for about twelve definitions instead of
thirty-one** — which is the amendment's whole argument, and it holds.
...
| `tests/enemies/enemyShape.test.ts` | `Enemy`'s non-optional fields equal exactly what `spawnEnemy` builds | Every field turned on in §6 must be added to `Enemy` in the same commit or this goes red — which is the point |
...
- **The boss fixture of §5.1**, before any brain is written.
...
## 10. Notes back to `docs/direction.md`
...
**10.1 The nine have no ranged ground enemy, and two placed letters have no
successor.** `A` (Mancubus, 260 hp, twin-barrel artillery) is the second most
placed letter in the kept levels at 9 tiles; `j` (Cultist, a dodging caster) is
placed once. Neither maps to any of the nine. The Censer answers *area denial*
and The Suspended answers *elevation*; nothing in the table answers *pressure
from a distance on the ground*. That may be deliberate — nine enemies with nine
distinct answers is the whole rule, and "ranged" is not by itself an answer. But
deleting `A` removes 9 of 73 non-boss tiles from the kept levels and takes the
game's only heavy with it, and that is worth an explicit yes rather than a
silent one.

**10.2 The kept levels place six distinct boss letters; the direction names
three, and only two of those six are among them.** Measured: `U` appears in all
four kept levels, `E` in two, and `Q`, `Z`, `N`, `V` in one each. The Cathedral
Guardian (`U`) is currently a *repeated miniboss*, not a signature encounter —
which sits oddly with "arena-interactive, phase mechanic tied to the pillars",
a thing you would not build four of. `E` (Mutant Executioner) has no successor
in the named three despite being placed in two kept levels, and
`direction.md`'s own level table cites "Executioner always at (3,2)" as evidence
of the copy-paste it is cutting. Where the Executioner goes, and whether the
Guardian appears once or four times, are level-composition decisions the roster
spec cannot make.

**10.3 The Cathedral Guardian has the least existing machinery of the three
...
**10.4 The Bellringer is the only one of the nine with nothing to inherit.**
...
Mitigation: §6 names the consequence of each one *before* it is turned on, so a
reviewer can tell a field that was decided from a field that was swept in.

**Rewriting `priestThink` with no trace coverage.** §5.1. Mitigation: the boss
...
Mitigation: §9's new assertion, and §7's decision made before the first def is
...
## 12. What this phase leaves broken
...
- **Six of KNOWN-15's ten fields remain dead or deleted** rather than fixed —
  ...
  what is left.
- **Levels 2-4 still share the copy-pasted layout** (Phase 4), whatever §7
  decides about their *contents*.
- **KNOWN-4 and KNOWN-11 are not closed**, though both change shape: see §7.
