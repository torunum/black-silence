...
/**
 * A thin bridge for `scene`, which still lives in src/legacy.js (the THREE
 * CORE section, owned by a later plan) but is needed by the FX modules
 * extracted in Phase 0C Task 2 (src/fx/Particles.ts, Decals.ts, Gibs.ts).
 *
 ...
 *
 * legacy.js calls setScene(scene) immediately after `scene=new THREE.Scene()`
 * in loadLevel(), before buildParticles()/resetDecals()/resetGibs() run, so
 * every FX module sees the current level's scene by the time anything in
 * this task's modules touches it.
 *
 ...
 * and one place to delete when Plan 0E eventually carves THREE CORE out of
 * legacy.js and scene ownership moves again.
 */
